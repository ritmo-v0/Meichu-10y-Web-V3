...
export default function YearPage(props) {
	const year = use(props.params).year;

	// TODO: Skeleton loading; year range protection
	const { data } = useSWR(year ? `about/${year}` : null, fetcher);
	const yearData = data || {};

	return (
		<div>
			<header className="mb-4">
				<Banner year={year} bannerUrl={yearData.bannerUrl} />
				...
				<article className="grid gap-y-32 [&_h2]:mb-12">
					{yearData?.description && (
						<section id="year-description" className="wrapper-md">
							<MarkdownParagraph content={yearData.description} />
						</section>
					)}

					{yearData?.competition && <Competition competition={yearData.competition} />}

					{yearData?.scenes && <Scenes scenes={yearData.scenes} />}

					{yearData?.topics && <Topics topics={yearData.topics} />}

					{yearData?.sponsors && <Sponsors sponsors={yearData.sponsors} />}

					{yearData?.staffs && <Staffs year={year} logoUrl={yearData?.logoUrl} staffs={yearData.staffs} />}
				</article>
			</main>
		</div>
	);
